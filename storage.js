...
export const KEY="janki.collection.v7", OLD_KEYS=["janki.collection.v6","janki.collection.v5","janki.collection.v4","janki.collection.v3"];
export function blank(){return {version:7,notes:[],cards:{},reviews:[],settings:defaultOptions()};}
...
function addMissingStarterCards(d){
  const existing=new Set(d.notes.map(n=>`${n.deck}\u0000${n.front}`));
  for(const [deck,arr] of Object.entries(seedDecks)) for(const a of arr){
    const key=`${deck}\u0000${a[0]}`;
    if(existing.has(key)) continue;
    const n=makeNote(a,deck); d.notes.push(n); d.cards[n.id]={...initialState(),noteId:n.id}; existing.add(key);
  }
  return d;
}
...
export function load(storage=localStorage){for(const key of [KEY,...OLD_KEYS]){try{const raw=storage.getItem(key);if(raw){const d=addMissingStarterCards(normalize(JSON.parse(raw)));storage.setItem(KEY,JSON.stringify(d));return d;}}catch{ /* try the next source */ }}const d=seed();storage.setItem(KEY,JSON.stringify(d));return d;}
